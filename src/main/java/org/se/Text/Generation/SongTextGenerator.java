...
public class SongTextGenerator {
	...
	private List<String> generateStrophe(Structure.Genre genre, List<Integer> usedStrophesList,
		...
		if (genre == Structure.Genre.blues) {
			...
			System.out.println(testVers.substring(0, beginning) + " Hier kommt das eingesetzte Wort: " + getTerm(strArr)
					+ " " + testVers.substring(end + 1));
	...
	private Integer getRandomNotUsedValue(List<Integer> List, int listLength) {
		...
		do {
			...
			if (terminateCounter < 0)
				return ((int) (Math.random() * (List.size() + 1))); // if there is no unused index left
	...
	private int getCorrectPosition(int Index, int position) {
		if (Index - position <= 0)
			return getCorrectPosition(Index, position - Index);
		return position;
	}
